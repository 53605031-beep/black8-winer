...
Page({
  ...
  async _checkMerchant() {
    try {
      const me = await cloudDB.getCurrentUser();
      if (!me || me.role !== "merchant" || !me.merchantVenueId) {
        wx.showModal({
          title: "无权限",
          content: "商家管理仅对已入驻商家开放",
          showCancel: false,
      ...
      this.setData({
        me,
        venueId: me.merchantVenueId,
        venueName: me.merchantVenueName || "我的球房"
      });
      await this._load();
    } catch (e) {
      console.error("检查商家身份失败", e);
      wx.showModal({
        title: "加载失败",
        content: "请检查网络后重试",
        showCancel: false,
  ...
  async _load() {
    ...
    try {
      if (this.data.tab === "goods") {
        const goods = await cloudDB.getMyVenueGoods(this.data.venueId);
        this.setData({ goodsList: goods || [] });
      } else {
        const list = await cloudDB.getMyVenueRedemptions(this.data.venueId);
  ...
  async onSaveGoods() {
    const { form, editGoods, venueId } = this.data;
    if (!form.name.trim()) {
    ...
    try {
      await cloudDB.saveGoods({ ...form, venueId }, editGoods?._id || null);
      wx.hideLoading();
