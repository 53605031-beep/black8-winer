...
async function getCurrentUser() {
  ...
  if (data.length === 0) {
    ...
    // 老用户迁移：只补缺失字段，不能把真实的 0 余额重置成初始约豆。
    const u = data[0];
    const yuedouPatch = {};
    if (u.yuedou == null) yuedouPatch.yuedou = YUEDOU_INITIAL;
    if (u.yuedouFrozen == null) yuedouPatch.yuedouFrozen = 0;
    if (u.yuedouSystem == null) yuedouPatch.yuedouSystem = 0;
...
async function claimDailyBonus() {
  ...
  return { code: out.code, remaining: out.remaining, totalClaimed: out.totalClaimed };
}
