...
async function _settleMatch(matchId, match, participants) {
  ...
  try {
    ...
    if (winnerId) {
      ...
      // 加入时 500 已从可用约豆转入冻结，这里只结算本局冻结资金。
      await db.collection("yueqiu8_users").where({ openid: winnerId }).update({
        data: { yuedou: db.command.inc(YUEDOU_FROZEN + YUEDOU_WINNER), yuedouFrozen: db.command.inc(-YUEDOU_FROZEN), yuedouSystem: db.command.inc(YUEDOU_SYSTEM) }
      });
      await db.collection("yueqiu8_users").where({ openid: loserId }).update({
        data: { yuedouFrozen: db.command.inc(-YUEDOU_FROZEN) }
      });
...
async function getCurrentUser() {
  ...
  if (data.length === 0) {
    ...
    // 老用户迁移：只补缺失字段，真实的 0 余额不能被当成“未初始化”
    const u = data[0];
    const needsFix = u.yuedou == null || u.yuedouFrozen == null || u.yuedouSystem == null;
    if (needsFix) {
      const fixData = {};
...
async function redeemGoods(goodsId, address = null) {
  ...
  return { success: true, message: out.message || "兑换成功", currencyType: out.currencyType };
}
