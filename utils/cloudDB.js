...
const { buildMissingYuedouUpdate } = require("./economyRules");
...
async function _settleMatch(matchId, match, participants) {
  throw new Error(`_settleMatch 已废弃，请通过 matchService.submitResult 结算：${matchId}`);
}
...
/**
 * 旧版直接改分接口已停用。比赛结果必须走 matchService.submitResult，
 * 由服务端事务完成余额结算。
 */
async function submitMatchResult(matchId, winnerId, loserId) {
  throw new Error(`submitMatchResult 已停用，请通过 submitResultChoice 提交：${matchId || ""}`);
}
...
async function getCurrentUser() {
  ...
  if (data.length === 0) {
    ...
    // 老用户迁移：只补缺失字段，不能把真实 0 余额当成未初始化。
    const u = data[0];
    const yuedouPatch = buildMissingYuedouUpdate(u);
...
async function getDailyBonusStatus() {
  ...
  if (openid) {
    try {
      const claimRecord = await DB().collection("daily_claims").doc(`${today}_${openid}`).get();
      claimed = !!claimRecord.data;
    } catch (_) {
      claimed = false;
    }
  }
  ...
  try {
    ...
    return { claimed, remaining: YUEDOU_DAILY_POOL, totalClaimed: 0 };
  }
}
